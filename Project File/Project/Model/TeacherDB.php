<?php 
    ...
    function edit($id, $name, $gender, $dob, $phone, $email) {
        $ezl = connect();
        $sql = "UPDATE customers SET Name='$name', Email='$email', Gender='$gender', DateOfBirth='$dob', Contact='$phone' WHERE ID=$id";
        $qry = $ezl->query($sql);
    ...
    function user($user, $id) {
        ...
        $sql = "UPDATE customers SET Username='$user' WHERE ID='$id'";
        $qry = $ezl->query($sql);
        //header('location: /Project/View/viewprofile.php');
        $ezl->close();
    }
    ...
    function insert_student($firstname, $lastname, $gender, $dob, $religion, $preaddress, $paraddress, $phone, $email, $username, $password) {
        ...
        $sql = "INSERT INTO managers(FirstName, LastName, Gender, DateOfBirth, Religion, PresentAddress, PermanentAddress, PhoneNo, Email, Username, Password) VALUES ('$firstname', '$lastname', '$gender', '$dob', '$religion', '$preaddress', '$paraddress', '$phone', '$email', '$username', '$password')";
